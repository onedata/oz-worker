...
upgrade_from_19_02_x(Config) ->
    ...
    lists:foreach(fun({UserId, TokenId, Secret, LegacyTokenSerialized}) ->
        ...
        ?assertMatch({true, ?USER(UserId)}, oz_test_utils:check_token_auth(Config, LegacyToken, undefined))
    ...
    lists:foreach(fun({ProviderId, TokenId, Secret, LegacyRootTokenSerialized}) ->
        ...
        % The migrated token should still work
        ?assertMatch({true, ?PROVIDER(ProviderId)}, oz_test_utils:check_token_auth(Config, LegacyRootToken, undefined))
