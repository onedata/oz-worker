...
%%% @end
%%%-------------------------------------------------------------------
-module(gr_datastore_test_SUITE).
-author("Rafal Slota").
...
cache_monitoring_test(Config) ->
    [Worker1, Worker2] = Workers = ?config(gr_nodes, Config),
...
local_cache_test(Config) ->
    [Worker1, Worker2] = ?config(gr_nodes, Config),
...
global_cache_list_test(Config) ->
    generic_list_test(?config(gr_nodes, Config), ?GLOBAL_ONLY_LEVEL).
...
init_per_testcase(Case, Config) when
    ...
    Case =:= link_walk_test ->
    Workers = ?config(gr_nodes, Config),
