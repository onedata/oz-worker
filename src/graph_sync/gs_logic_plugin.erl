...
%% @end
%%--------------------------------------------------------------------
-spec client_heartbeat(aai:auth(), gs_server:conn_ref()) -> ok.
client_heartbeat(?PROVIDER(ProvId) = Client, _ConnectionRef) ->
    provider_connections:report_heartbeat(ProvId);
...
is_type_supported(#gri{type = space_stats}) -> true;
