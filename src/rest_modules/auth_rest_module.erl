...
accept_resource(Resource, post, Id, Data, _Client, Req)
    ...
    TokenClient = case Resource of ptokens -> {provider, Id}; ctokens ->
        native end,
                 ...
                 <<"authorization_code">> ->
                     ...
                     ClientName =
                         case proplists:get_value(<<"client_name">>, Data) of
                             ...
                             undefined when Resource =:= ctokens ->
                                 rest_module_helper:report_missing_key(<<"client_name">>, Req);
                             Value when is_binary(Value) -> Value;
                             Value ->
                                 rest_module_helper:report_invalid_value(<<"client_name">>, Value, Req)
                         end,
    ...
    case Result of
        ...
        {error, Reason} ->
            Description1 = case Reason of
                               refresh_invalid_or_revoked ->
                                   <<"refresh token invalid or revoked">>;
                               refresh_wrong_client ->
