...
is_authorized(handles, get, HandleServiceId, #client{type = user, id = UserId}) ->
    handle_logic:has_effective_privilege(HandleServiceId, UserId, view_handle);
...
is_authorized(handle, patch, HandleServiceId, #client{type = user, id = UserId}) ->
    handle_logic:has_effective_privilege(HandleServiceId, UserId, modify_handle);
is_authorized(handle, delete, HandleServiceId, #client{type = user, id = UserId}) ->
    handle_logic:has_effective_privilege(HandleServiceId, UserId, delete_handle);
...
resource_exists(handle, HandleServiceId, Req) ->
    {handle_logic:exists(HandleServiceId), Req};
...
accept_resource(handles, post, _HandleServiceId, Data, #client{type = user, id = UserId}, Req) ->
    ...
    {ok, HandleServiceId} = handle_logic:create(UserId, HandleServiceId, ResourceType, ResourceId),
    {{true, <<"/handles/", HandleServiceId/binary>>}, Req};

accept_resource(handle, patch, HandleServiceId, Data, #client{type = user, id = _UserId}, Req) ->
    ok = handle_logic:modify(HandleServiceId, Data),
    {true, Req};
...
accept_resource(user, put, HandleServiceId, _Data, _Client, Req) ->
    ...
    case user_logic:exists(UID) of
        ...
        true ->
            {ok, HandleServiceId} = handle_logic:add_user(HandleServiceId, UID),
            {true, Req2}
    end;
accept_resource(group, put, HandleServiceId, _Data, _Client, Req) ->
    ...
    case group_logic:exists(GID) of
        false ->
            ...
            rest_module_helper:report_error(invalid_request, Description, Req2);
        true ->
            {ok, HandleServiceId} = handle_logic:add_group(HandleServiceId, GID),
            {true, Req2}
    end;
accept_resource(upriv, put, HandleServiceId, Data, _Client, Req) ->
    ...
    ok = handle_logic:set_user_privileges(HandleServiceId, UID, Privileges),
    {true, Req2};
accept_resource(gpriv, put, HandleServiceId, Data, _Client, Req) ->
    ...
    Privileges = [binary_to_existing_atom(P, latin1) || P <- BinPrivileges],
    ok = handle_logic:set_group_privileges(HandleServiceId, GID, Privileges),
    {true, Req2}.
...
provide_resource(handles, _EntityId, #client{type = user, id = UserId}, Req) ->
    {ok, HandleServiceIds} = handle_logic:list(UserId),
    {[{handles, HandleServiceIds}], Req};
provide_resource(handle, HandleServiceId, #client{type = user, id = _UserId}, Req) ->
    {ok, Data} = handle_logic:get_data(HandleServiceId),
    {Data, Req};
provide_resource(users, HandleServiceId, _Client, Req) ->
    {ok, Users} = handle_logic:get_users(HandleServiceId),
    {Users, Req};
provide_resource(upriv, HandleServiceId, _Client, Req) ->
    {UID, Req2} = cowboy_req:binding(uid, Req),
    {ok, Privileges} = handle_logic:get_user_privileges(HandleServiceId, UID),
    {[{privileges, Privileges}], Req2};
provide_resource(groups, HandleServiceId, _Client, Req) ->
    {ok, Groups} = handle_logic:get_groups(HandleServiceId),
    {Groups, Req};
provide_resource(gpriv, HandleServiceId, _Client, Req) ->
    {GID, Req2} = cowboy_req:binding(gid, Req),
    {ok, Privileges} = handle_logic:get_group_privileges(HandleServiceId, GID),
    {[{privileges, Privileges}], Req2}.
...
delete_resource(handle, HandleServiceId, Req) ->
    {handle_logic:remove(HandleServiceId), Req};
delete_resource(user, HandleServiceId, Req) ->
    {UID, Req2} = cowboy_req:binding(uid, Req),
    {handle_logic:remove_user(HandleServiceId, UID), Req2};
delete_resource(group, HandleServiceId, Req) ->
    {GID, Req2} = cowboy_req:binding(gid, Req),
    {handle_logic:remove_group(HandleServiceId, GID), Req2}.