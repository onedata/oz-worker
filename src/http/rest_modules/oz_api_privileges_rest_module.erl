...
%%%-------------------------------------------------------------------
-module(oz_api_privileges_rest_module).
-author("Lukasz Opiola").
...
routes() ->
    ...
    M = rest_handler,
    [
        {<<"/oz_api_privileges/users/:id">>, M, S#rstate{resource = onedata_user, methods = [get, put, delete]}},
        {<<"/oz_api_privileges/groups/:id">>, M, S#rstate{resource = user_group, methods = [get, put, delete]}}
    ].
...
-spec is_authorized(Resource :: resource(), Method :: method(),
    ...
    boolean().
is_authorized(onedata_user, _, _EntityId, #client{id = UserId}) ->
    oz_api_privileges_logic:has_effective_privilege(UserId, set_privileges);
is_authorized(user_group, _, _EntityId, #client{id = UserId}) ->
    oz_api_privileges_logic:has_effective_privilege(UserId, set_privileges);
...
accept_resource(EntityType, put, EntityId, Data, _Client, Req) ->
    % Resource is equal to EntityType (onedata_user | user_group)
...
provide_resource(EntityType, EntityId, _Client, Req) ->
    % Resource is equal to EntityType (onedata_user | user_group)
...
-spec delete_resource(Resource :: removable_resource(),
    ...
    {boolean(), cowboy_req:req()}.
delete_resource(EntityType, EntityId, Req) ->
    % Resource is equal to EntityType (onedata_user | user_group)
