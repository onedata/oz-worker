...
%   * gui_login - a login process using OpenID/SAML
%   * access_token - a user has presented an access token issued by
%                    an IdP in another interface (e.g. REST API)
%   * refresh_token - an access token that is cached is being refreshed
%                     using a refresh token
-type flow_type() :: gui_login | access_token | refresh_token.
...
validate_login(Method, Req) ->
    ...
    case state_token:lookup(StateToken) of
        error ->
            ...
            idp_auth_logger:log_error(?ERROR_INVALID_STATE, undefined, gui_login, StateToken, []),
        ...
        {ok, #{idp := IdP, redirect_after_login := RedirectAfterLogin} = StateInfo} ->
            try validate_login_by_state(Payload, StateToken, StateInfo) of
                ...
                {error, Reason} ->
                    idp_auth_logger:log_error({error, Reason}, IdP, gui_login, StateToken, []),
                    {auth_error, {error, Reason}, StateToken, RedirectAfterLogin}
            catch
                throw:{error, _} = Error:Stacktrace ->
                    idp_auth_logger:log_error(Error, IdP, gui_login, StateToken, Stacktrace),
                    {auth_error, Error, StateToken, RedirectAfterLogin};
                Type:Reason:Stacktrace ->
                    idp_auth_logger:log_error({Type, Reason}, IdP, gui_login, StateToken, Stacktrace),
...
refresh_idp_access_token(IdP, RefreshToken) ->
    try
        ...
        throw:ThrownError:Stacktrace ->
            idp_auth_logger:log_error(ThrownError, IdP, refresh_token, <<"refresh_token_flow">>, Stacktrace),
            ?ERR_INTERNAL_SERVER_ERROR(?err_ctx(), undefined);
        Type:Reason:Stacktrace ->
            idp_auth_logger:log_error({Type, Reason}, refresh_token, IdP, <<"refresh_token_flow">>, Stacktrace),
            ?ERR_INTERNAL_SERVER_ERROR(?err_ctx(), undefined)
    end.
