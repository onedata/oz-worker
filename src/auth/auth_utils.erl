...
%% 3. {error, ErrorId, RedirectPage} if the validation failed or any other error occurred.
%% @end
%%--------------------------------------------------------------------
-spec validate_login() -> new_user | {redirect, URL :: binary()} |
{error, ErrorId :: atom(), RedirectPage :: binary()}.
validate_login() ->
    try
        ...
        case StateInfo of
            error ->
                ...
                {error, ?error_auth_invalid_request, <<?page_login>>};
            ...
            Props ->
                ...
                ConnectAccountFlow = proplists:get_value(connect_account, Props),
                ErrorRedirectPage = case ConnectAccountFlow of
                    true -> <<?page_after_login_expand_accounts>>;
                    false -> <<?page_login>>
                end,
                ...
                case Module:validate_login() of
                    {error, Reason} ->
                        ...
                        {error, ?error_auth_invalid_request, ErrorRedirectPage};
                    ...
                    {ok, OriginalOAuthAccount = #oauth_account{provider_id = ProviderId, user_id = UserID, email_list = OriginalEmails, name = Name}} ->
                        ...
                        Result = case ConnectAccountFlow of
                            false ->
                                    ...
                                    _ ->
                                        ...
                                        case is_any_email_in_use(Emails, <<"">>) of
                                            true ->
                                                ...
                                                {error, ?error_auth_new_email_occupied, ErrorRedirectPage};
                                            false ->
                            ...
                            true ->
                                ...
                                case user_logic:get_user({connected_account_user_id, {ProviderId, UserID}}) of
                                    {ok, _} ->
                                        ...
                                        {error, ?error_auth_account_already_connected, ErrorRedirectPage};
                                    _ ->
                                        % Not found, ok
                                        ...
                                        case is_any_email_in_use(Emails, g_session:get_user_id()) of
                                            true ->
                                                ...
                                                {error, ?error_auth_connect_email_occupied, ErrorRedirectPage};
                                            false ->
                                                ...
                                                {redirect, <<?page_after_login_expand_accounts>>}
                                        end
                                end
                        end,
                        Result
