...
%% API
-export([get_login_endpoint/3, validate_login/2]).
...
-spec get_login_endpoint(auth_config:idp(), LinkAccount :: false | {true, od_user:id()},
    RedirectAfterLogin :: binary()) -> {ok, maps:map()} | {error, term()}.
get_login_endpoint(IdP, LinkAccount, RedirectAfterLogin) ->
    try
        StateToken = auth_tokens:generate_state_token(IdP, LinkAccount, RedirectAfterLogin),
